...
def angleCorrect(image, normalZYX, centerZYX, method='linear', spacingZYX=None, targetSpacingZYX=None, spacingBack=False):
	'''
	...
	if targetSpacingZYX is not None:
		...
		targetSpacingZYX = [targetSpacingZYX[idx] if targetSpacingZYX[idx] is not None else spacingZYX[idx] for idx in range(3)] # 如果其中某项为None，则表示该维度不进行spacing转换，维持原spacing不变
...
def saveRotatedImage(image, imageSavePath, w, b, centerZYX, spacingZYX, targetSpacingZYX=None, angleError=None, drawPlaneValue=None, spacingBack=False):
	...
	rotatedImage, transformMatrix = angleCorrect(image, normalZYX, centerZYX, spacingZYX=spacingZYX, targetSpacingZYX=targetSpacingZYX, spacingBack=spacingBack)
...
def checkRotateResult(rotatedImage, clipRange=(0, 300), downSampleRatio=16, errorThreshold=100):
	'''
	判断旋转后的图像是否是近似左右对称
	'''
	Z, H, W = rotatedImage.shape
	centerSliceIdx = Z // 2
	centerSlice = rotatedImage[centerSliceIdx].copy()
	
	centerSlice = np.clip(centerSlice, clipRange[0], clipRange[1])

	centerSlice -= clipRange[0]
	centerSlice = centerSlice.astype(np.float32)
	centerSlice /= clipRange[1] - clipRange[0]

	scale = 1.0 / downSampleRatio

	centerSlice = scipy.ndimage.zoom(centerSlice, (scale, scale), order=1, mode='constant', cval=0)

	subMap = np.abs(centerSlice - centerSlice[:, ::-1])

	error = np.sum(subMap)

	print('checked error:', error)
...
if __name__ == '__main__':
	...
	dstDir    = '/mnt/data3/brain_angle_correction/result_no_rotate_line_masks_gray_scale1_shift1_allImage' # 完成处理之后，用于保存结果的文件夹
	# dstDir    = '/mnt/data3/brain_angle_correction/result_no_rotate_line_masks_gray_scale1_shift1_CT_lianying_order1'
	...
	for imagePath in imagePaths:
		...
		if normalGTDict is not None:
			...
			saveRotatedImage(image, imageSavePath, w, b, centerZYX, spacingZYX, targetSpacingZYX=targetSpacingZYX, angleError=angleError, drawPlaneValue=9999 if showPlane else None, spacingBack=spacingBack)
		else:
			saveRotatedImage(image, imageSavePath, w, b, centerZYX, spacingZYX, targetSpacingZYX=targetSpacingZYX, angleError=None, drawPlaneValue=9999 if showPlane else None, spacingBack=spacingBack)
