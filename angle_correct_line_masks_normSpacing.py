...
def angleCorrect(image, normalZYX, centerZYX, method='linear'):
	'''
	...
	'''
	normalZYX = np.array(normalZYX, dtype=np.float)
	centerZYX = np.array(centerZYX, dtype=np.float)
	# 首先要将中心点移到原点位置
	...
	# 计算旋转矩阵
	rotateMatrix = getRotateMatrix3D(normalZYX)
	# 将旋转矩阵扩展为齐次形式
	...
	deTranslateMatrix = np.array([
		[1, 0, 0, imageShape[0] / 2.0],
		[0, 1, 0, imageShape[1] / 2.0],
		[0, 0, 1, imageShape[2] / 2.0],
		[0, 0, 0, 1],
	])
	# 完整的变换矩阵: deTranslateMatrix * rotateMatrix * translateMatrix
	transformMatrix = deTranslateMatrix.dot(rotateMatrix.dot(translateMatrix))
	# 获取逆变换矩阵
	deTransformMatrix = np.linalg.inv(transformMatrix)

	# 构造变换之后的坐标网格，和原图大小一致
	gridZ, gridY, gridX = np.mgrid[range(imageShape[0]), range(imageShape[1]), range(imageShape[2])]
	# 拓展为齐次坐标
	grid1 = np.ones(imageShape) # (Z, H, W)
	...
	# 原图上的坐标网格
	points = (range(imageShape[0]), range(imageShape[1]), range(imageShape[2]))
	# 需要插值的点
	xi     = (oriZYX[..., 0], oriZYX[..., 1], oriZYX[..., 2])
	# 进行插值，需要插值的地方如果超出图像外，则用最小值代替
	interpolatedImg = scipy.interpolate.interpn(points, image, xi, method=method, bounds_error=False, fill_value=image.min())
...
def getCentralPlane(image, model, inputLayerNums=24, cuda=True, predImageSaveDir=None, spacingZYX=None):
	...
	for i, index in enumerate(indices):
		# 每一层都有一个mask
		# 首先变回原始大小
		layerPred = transform.resize(yPred[i], (image.shape[1], image.shape[2]), mode='constant', cval=0, anti_aliasing=True, preserve_range=True)
		...
		# 如果该层预测出来的mask小于200个像素点大于0.99，则放弃这一层的点
		if centerPoints.shape[0] < 200:
			continue
		else:
			centerPoints = centerPoints[np.random.choice(centerPoints.shape[0], size=200, replace=False)] # 大于200个点的话，随机取其中200个点，以免不同层之间点数差距太大
...
def saveRotatedImage(image, imageSavePath, w, b, centerZYX, spacingZYX, targetSpacingZYX=None, angleError=None, drawPlaneValue=None):
	...
	if targetSpacingZYX is not None:
		# 插值，将图片spacing换到目标spacing, 法向量w和中心点坐标也要做相应变换
		scaleZYX = (spacingZYX[0] / targetSpacingZYX[0], spacingZYX[1] / targetSpacingZYX[1], spacingZYX[2] / targetSpacingZYX[2])
		
		normalZYX = [w[idx] / scaleZYX[idx] for idx in range(3)]
		centerZYX = [centerZYX[idx] * scaleZYX[idx] for idx in range(3)]

		# 调整图像spacing
		image = scipy.ndimage.zoom(image, scaleZYX, order=0, mode='constant', cval=image.min())
	...
	print('save image:', imageSavePath, image.shape, image.max(), image.min(), image.dtype, 'normalZYX:', normalZYX, 'centerZYX:', centerZYX)

	if drawPlaneValue is not None:
		# 将平面画出来方便查看
		drawPlane(image, normalZYX, b, drawPlaneValue)
	
	# 进行图像旋转
	rotatedImage = angleCorrect(image, normalZYX, centerZYX)
	endTime = time.time()
...
if __name__ == '__main__':
	...
	# imageDir  = '/mnt/data3/brain_angle_correction/CT_lianying' # 所有待测试数据放到这个文件夹下
	modelPath = '/mnt/data3/brain_angle_correction/model_save/no_rotate_line_masks/unet_modify_group.pkl' #'/mnt/data1/wx/graduation_project/model/unet_modify_group.pkl' # '/mnt/data3/brain_angle_correction/model_save/no_rotate/unet_modify_group.pkl' # 模型文件路径 '/mnt/data1/wx/result_project/model/unet_modify_group.pkl'
	dstDir    = '/mnt/data3/brain_angle_correction/result_no_rotate_line_masks' # 完成处理之后，用于保存结果的文件夹
	# dstDir    = '/mnt/data3/brain_angle_correction/result_no_rotate_line_masks_CT_lianying'
	normalGTPath  = '/mnt/data3/brain_angle_correction/GT_2_26.npy' # None # 
	showPlane = True
	...
	for imagePath in imagePaths:
		...
		w, b = getCentralPlane(image, model, inputLayerNums=24, cuda=True, predImageSaveDir=predImageSaveDir, spacingZYX=spacingXYZ) # 这里的输出w就是法向量
		# 获取中心点坐标
		...
		if normalGTDict is not None:
			...
			normalGT = normalGTDict[mhdName]
			angleErrorBeforeNormSpacing = calVectorIncludeAngle(w, normalGT) # 这里计算的是spacing没有统一的角度差异，由于z轴spacing一般远大于x和y轴，因此微小的角度差异会被放大
			...
			print(mhdName, 'angle with gt:{:.2f}, angle with gt before norm spacing:{:.2f}'.format(angleError, angleErrorBeforeNormSpacing))
			
			if angleError >= 5:
				saveRotatedImage(image, imageSavePath, w, b, centerZYX, spacingZYX, targetSpacingZYX=targetSpacingZYX, angleError=angleError, drawPlaneValue=9999 if showPlane else None)
		else:
			saveRotatedImage(image, imageSavePath, w, b, centerZYX, spacingZYX, targetSpacingZYX=targetSpacingZYX, angleError=None, drawPlaneValue=9999 if showPlane else None)
			
	if normalGTDict is not None:
		print('mean angle error: {:.2f}, mean angle error before norm spacing: {:.2f}'.format(np.mean(angleErrors), np.mean(angleErrorsBeforeNormSpacing)))