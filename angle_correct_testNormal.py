...
def imagePreProcess(image, normalizeRange=None, inputLayerNums=24):
	...
	if image.shape[1] != 512 or image.shape[2] != 512:
		# 使用最邻近插值方式，但是在插值之前有高斯模糊过程以抗锯齿
		image = transform.resize(image, (inputLayerNums, 512, 512), order=0, mode='constant', cval=0, anti_aliasing=True)

	return image, indices


def getCentralPlane(image, model, inputLayerNums=24, cuda=True):
	'''
	使用模型获取图像中平面参数
	image: 图像数组
	model: 用于预测的模型
	...
	with torch.no_grad():
		...
		for i, index in enumerate(indices):
			# 每一层都有一个mask
			# 首先变回原始大小
			layerPred = transform.resize(yPred[i], (image.shape[1], image.shape[2]), mode='constant',anti_aliasing=True)
			coordinate = np.array(np.where(layerPred > 0.95)).transpose((1, 0)) # [(y1, x1), (y2, x2), ...]
			# print('coordinate.shape', coordinate.shape)
			# 如果该层预测出来的mask小于2个像素点大于0.95，则放弃这一层的点
			if coordinate.shape[0] < 2:
				continue
			# 使用kmeans聚类得到两个中心，这个就是关键点
			center = KMeans(n_clusters=2, random_state=0).fit(coordinate).cluster_centers_
			# 如果关键点距离太小，则放弃这一层的关键点
			if (abs(center[0][1] - center[1][1]) + abs(center[0][0] - center[1][0])) < 50:
				continue
			for j in range(center.shape[0]):
				points.append([index, center[j][0], center[j][1]])
		# 聚类之后的一个区域的点
		# 聚类得到的点少于4个,异常处理
		if len(points) < 4:
			print('预测出的points太少：', points)
			return None, None
		...
		linereg.fit(X, Y)
		# 获取平面方程的系数， z = w[0] * y + w[1] * x + b
		w = linereg.estimator_.coef_  # w[0]是y的系数，w[1]是x的系数
		b = linereg.estimator_.intercept_
	
	# 对参数进行调整，使得平面方程表示为：w[0] * z + w[1] * y + w[2] * x + b = 0
	w = [1, -w[0], -w[1]] # 同时这个也是法向量，zyx顺序
	b = -b
	return w, b
...
if __name__ == '__main__':
	...
	imageDir  = 'E:\\brain_angle_correction\\test\\images' # 所有待测试数据放到这个文件夹下
	# modelPath = '/mnt/data3/brain_angle_correction/model_save/no_rotate/unet_modify_group.pkl' # 模型文件路径 '/mnt/data1/wx/result_project/model/unet_modify_group.pkl'#
	dstDir    = 'E:\\brain_angle_correction\\no_spacing_order3' # 完成处理之后，用于保存结果的文件夹
	normalGTPath  = './GT_2_26.npy'
	...
	for imagePath in imagePaths:
		# if '900' not in imagePath:
		# 	continue
		mhdName = os.path.basename(imagePath)

		# 读取ground truth计算出来的法向量
		w = normalGTDict[mhdName][:3]
		b = normalGTDict[mhdName][3]
		...
		# w, b = getCentralPlane(image, model, inputLayerNums=24, cuda=True) # 这里的输出w就是法向量
		# if w is None:
		# 	print(imagePath, '预测中平面参数失败, 跳过该图像')
		# 	continue
		
		# angleError = calVectorIncludeAngle(w, normalGT)
		# angleErrors.append(angleError)
		# print(mhdName, 'w', w, 'gt', normalGT, 'angle with gt:', angleError)

		# 插值，将图片spacing换到目标spacing, 法向量w也要做相应变换
		scaleZYX = (spacingZYX[0] / targetSpacingZYX[0], spacingZYX[1] / targetSpacingZYX[1], spacingZYX[2] / targetSpacingZYX[2])
		# print(scaleZYX)
		image = scipy.ndimage.zoom(image, scaleZYX, order=3, mode='constant', cval=image.min())

		normalZYX = [w[idx] / scaleZYX[idx] for idx in range(3)]
		newSpacing = targetSpacingZYX # 这里不是很精确，因为变换之后的shape肯定是取整数的，如果后面要做精确操作，这里的新spacing不能这样直接赋值
		
		# 获取中心点坐标
		centerZYX = findCentrePoint(image, normalZYX, b)
		# centerZYX = 

		print(imagePath, image.shape, image.max(), image.min(), image.dtype, 'normalZYX:', normalZYX, 'centerZYX:', centerZYX)

		if showPlane:
			# 将平面画出来方便查看
			drawPlane(image, normalZYX, b, 4096)
		
		# 进行图像旋转
		rotatedImage = angleCorrect(image, normalZYX, centerZYX)
		endTime = time.time()
		print('用时：', endTime - startTime)
		# 保存原始图像(spacing有调整)以及旋转之后的图像
		imageSavePath = os.path.join(dstDir, os.path.basename(imagePath))
		# saveMhd(image, origin, newSpacing, imageSavePath)
		scaleZYX = [1.0 / scale for scale in scaleZYX]
		# print(scaleZYX)
		rotatedImage = scipy.ndimage.zoom(rotatedImage, scaleZYX, order=3, mode='constant', cval=image.min())
		saveMhd(rotatedImage, origin, spacingXYZ, imageSavePath.replace('.mhd', '_rotated.mhd'))
	
	# print('mean angle error', np.mean(angleErrors))