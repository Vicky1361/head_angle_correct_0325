...
def drawLine(image, z, k, b, value):
    coordY = np.array(range(image.shape[1]))
    coordX = k[0] * coordY + b
    coordY = np.round(coordY).astype(np.int)
    coordX = np.round(coordX).astype(np.int)
...
def drawLine2(image, z, k, b, value):
    coordX = np.array(range(image.shape[0]))
    coordY = k[0] * coordX + b
    coordY = np.round(coordY).astype(np.int)
    coordX = np.round(coordX).astype(np.int)
    image[z, coordY, coordX] = value
...
mask_dir = r'E:\Work Desktop\data_processed\mask'
target_dir = r'E:\Work Desktop\data_processed\mask_convert_3_9'
...
for path in mask_path:
    ...
    # print(maskSavePath, 'normalZYX', normalYX, 'b', b)
