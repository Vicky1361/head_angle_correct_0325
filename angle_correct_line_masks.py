...
def split(scan):
    '''
	去除头部以外的层，返回剩下层的index list,联影的数据还没有超过50层的
	'''
...
def imagePreProcess(image, normalizeRange=None, inputLayerNums=24):
    ...
    if image.shape[0] > 50:
        indexRange = split(image)  # 现在联影的数据基本都不会超过50层
    else:
    ...
    if image.shape[1] != 512 or image.shape[2] != 512:
        # 使用最邻近插值方式,preserve_range=True将这个参数置为true不然resize后会改变数据的类型
...
def getCentralPlane(image, model, inputLayerNums=24, cuda=True, predImageSaveDir=None, spacingZYX=None):
    ...
    imageC, indices = imagePreProcess(imageC, normalizeRange=(0, 300), inputLayerNums=inputLayerNums)  # 数据预处理
    ...
    points = []
    # 对每一层进行处理，i表示预测的层数，index表示在原图中的层数索引
    validLayerNums = 0
    for i, index in enumerate(indices):
        ...
        if predImageSaveDir is not None:
            # 保存预测结果
            saveImage = (np.round(layerPred * 255)).astype(np.uint8)  # 将预测的结果归一化到[0-255]
        ...
        if centerPoints.shape[0] < 100:
            # 如果该层预测出来的mask像素点大于0.99的小于100个，则放弃这一层的点
            continue
        else:
    ...
    if validLayerNums < 8:  # 大于100个像素点的不够8层
...
if __name__ == '__main__':
    ...
    imageDir = '/mnt/data1/wx/head_angle_correct/UNITED_IMAGING_DATA/CT_MHD'  # CT数据增加到50个
    ...
    dstDir = '/mnt/data1/wx/head_angle_correct/UNITED_IMAGING_DATA/CT_predict_50_2'  # 预测结果保存在这个文件夹下
    normalGTPath = None  # '/mnt/data3/brain_angle_correction/GT_2_26.npy' # None #
    # normalGTPath  = './GT_3_16.npy' # None #
    showPlane = False  # 不画中平面
    ...
    for imagePath in imagePaths:
        ...
        spacingZYX = list(reversed(spacingXYZ))  # 由XYZ坐标顺序转为ZYX

        if savePredImage:
            predImageSaveDir = os.path.join(dstDir, 'predImage', mhdName.split('.mhd')[0])  # 拼接预测文件的病例号目录
        else:
            predImageSaveDir = None

        # 现在使用12层的数据来训练的,相应地测试集的处理也改变成12层
        ...
        imageSavePath = os.path.join(dstDir, os.path.basename(imagePath).replace('.mhd', '_rotated.mhd'))  # 改变文件的后缀
        ...
        else:  # 没有金标准的联影数据,直接全部保存，后续再做的检查
            saveRotatedImage(image, imageSavePath, w, b, centerZYX, spacingZYX, targetSpacingZYX=targetSpacingZYX,
